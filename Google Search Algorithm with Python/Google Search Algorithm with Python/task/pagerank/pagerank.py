...
class PageRank:
    def __init__(self):
        ...
        self.L = np.array(my_list)
        self.query = input()
        self.page_rank = None

    def calculate_r_vector(self):
        J = np.ones((self.dim, self.dim))
        M = self.d * self.L + (1 - self.d) / self.dim * J
        r_0 = 100 * np.ones(self.dim) / self.dim
        r_1 = np.matmul(M, r_0)
        while la.norm(r_1 - r_0) > 0.009:
            r_0 = r_1
            r_1 = np.matmul(M, r_0)
        self.page_rank = r_1
    ...
    def search(self):
        ...
        if match is not None:
            self.page_rank = np.delete(self.page_rank, match)
            print(match_site)
