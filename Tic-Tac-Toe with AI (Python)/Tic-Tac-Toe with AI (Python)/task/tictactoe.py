...
class TicTacToe:
    ...
    def make_easy_move(self):
        ...
        free_cells = [(i, j) for i, x in enumerate(self.game) for j, y in enumerate(x) if y == ' ']
    ...
    def make_medium_move_helper(self):
        playing = []
        if np.count_nonzero(self.game == 'X') == np.count_nonzero(self.game == 'O'):
            playing = ['X', 'O']
        else:
    ...
    def minimax(self, player, minmax, game, cell):
        ...
        if result:
            ...
            free_cells = [(i, j) for i, x in enumerate(new_game) for j, y in enumerate(x) if y == ' ']
            scores = []
            new_player = 'X'
            if player == 'X':
                new_player = 'O'
    ...
    def make_hard_move(self):
        ...
        if np.count_nonzero(self.game == ' ') > 8:
            self.make_medium_move_helper() # random move on empty board
        else:
            if np.count_nonzero(self.game == 'X') == np.count_nonzero(self.game == 'O'):
                playing = ['X', 'O']
            else:
